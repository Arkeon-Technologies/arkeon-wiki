...
const INGESTOR_SYSTEM = [
  ...
  "",
  "── Tool semantics ─────────────────────────────────────────────",
  "edit_file is the only mutation. Set `mode` explicitly. Five modes:",
  "  mode='create'         {path, content}",
  "                        New file. Frontmatter + body in `content`.",
  "                        Fails if the path already exists.",
  "  mode='append'         {path, content}",
  "                        Add `content` at the end of an existing file.",
  "  mode='replace'        {path, search, replace}",
  "                        Aider-style SEARCH/REPLACE — `search` must",
  "                        match exactly once. Use for surgical edits",
  "                        like swapping a frontmatter line.",
  "  mode='annotate'       {path, insert_after_phrase, insert_text}",
  "                        Splice `insert_text` immediately after a",
  "                        unique anchor phrase. Everything else stays",
  "                        byte-for-byte identical. PREFER this over",
  "                        replace when you are adding an observation",
  "                        to an existing wiki — the schema makes it",
  "                        impossible to drift surrounding prose.",
  "  mode='delete_section' {path, heading}",
  "                        Remove an ATX heading line and its body up",
  "                        to the next same-or-higher heading. Pass",
  "                        the heading verbatim, e.g. '## Open threads'.",
  "There is no overwrite. There is no whole-file delete here.",
].join("\n");
...
const INGESTOR_WRITE_PROMPT = [
  ...
  "    material EXTENDS that section's topic. If yes, use mode='replace'",
  "    to swap the section's prose for an enriched version that weaves",
  "    in the new facts with inline citations, preserving every prior",
  "    citation untouched. For a small additive observation that fits",
  "    inside an existing paragraph (a phrase, a date, an inline",
  "    citation), prefer mode='annotate' — pick a unique anchor phrase",
  "    in the body and splice the new fragment after it. ANNOTATE is",
  "    the right shape when the surrounding prose should not change.",
  "    If the new material is a topic the wiki doesn't cover yet, use",
  "    mode='append' to add a new topical section.",
  "",
  "    If the existing wiki is structured as per-source sections (e.g.",
  "    \"## In Book I of Confessions\", \"## In Book II of Confessions\"),",
  "    REFACTOR it before adding your new material. Use mode='replace'",
  "    to swap the per-source sections for topical ones (\"## Early life\",",
  ...
  "",
  "  - [NEW] subjects: edit_file with mode='create'. Frontmatter must include",
...
const CONSOLIDATOR_SYSTEM = [
  ...
  "",
  "── Tool semantics ────────────────────────────────────────────────",
  "edit_file modes (set `mode` explicitly):",
  "  mode='append'         {path, content}",
  "                        Add to the end of an existing wiki. Allowed",
  "                        on ANY wiki under wiki/ — this is your only",
  "                        write tool for OTHER wikis.",
  "  mode='annotate'       {path, insert_after_phrase, insert_text}",
  "                        Splice text after a unique anchor phrase,",
  "                        leaving everything else byte-identical.",
  "                        Use on OTHER wikis when adding a one-line",
  "                        observation or a citation tied to specific",
  "                        existing prose, instead of opening a whole",
  "                        new section.",
  "  mode='replace'        {path, search, replace}",
  "                        SEARCH/REPLACE on the SUBJECT only. Used to",
  "                        wrap a label with a markdown link, trim",
  "                        bled material, or refactor the subject body.",
  "  mode='delete_section' {path, heading}",
  "                        Remove an ATX heading and its body up to the",
  "                        next same-or-higher heading. SUBJECT only.",
  "  mode='create'         You won't create new wikis. That's the",
  "                        ingestor's job.",
  "",
  ...
  "  - edit_file mode='replace' or 'delete_section' only on the subject",
  "    wiki ({{trigger_path}}).",
  "  - edit_file mode='append' or 'annotate' on any wiki under `wiki/`.",
].join("\n");
...
const CONSOLIDATOR_EDIT_PROMPT = [
  ...
  "",
  "CROSS_LINK:",
  "  edit_file mode='replace' on the SUBJECT ({{trigger_path}}). The other",
  ...
  "",
  "BLEED_INTO:",
  "  Step 1 — edit_file mode='append' on the OTHER wiki. Synthesize a new",
  ...
  "",
  "  Step 2 — edit_file mode='replace' on the SUBJECT. Trim the bled",
  ...
  "  the subject still finds the material. (mode='delete_section' on",
  "  the heading is also fine when no pointer is needed.) Preserve",
  "  the rest of the subject's body untouched.",
  "",
  "MERGE_INTO:",
  "  Step 1 — edit_file mode='append' on the OTHER wiki. Synthesize the",
  ...
  "",
  "Hard rules:",
  "  - You may DELETE only the subject ({{trigger_path}}) — both",
  "    delete_wiki and edit_file mode='delete_section'.",
  "  - You may REPLACE content (mode='replace') only on the subject.",
  "  - You may APPEND (mode='append') or ANNOTATE (mode='annotate') on",
  "    any wiki under wiki/.",
