...
const INGESTOR_GATHER_PROMPT = [
  ...
  "   — ranked by similarity. A hit with similarity ≳ 0.6 whose body is",
  "   plausibly about the same subject as yours is a match, even if the",
  "   labels differ. Treat it as [EXISTS]; you already have the body in",
  "   the response, so phase 2 sees its current shape and prior citations",
  "   without an extra read_file.",
  "",
