...
describe("/wikis filters/counts driven by contributions", () => {
  it("has_contributions=true excludes wikis with no contributions", async () => {
    // Seed a wiki authored on disk with no contributions array.
    mkdirSync(join(testDir, "wiki/concept"), { recursive: true });
    const fm = yaml
      .dump(
        { label: "No Contribution Wiki", subject_type: "concept" },
        { schema: yaml.JSON_SCHEMA, sortKeys: false },
      )
      .trimEnd();
    writeFileSync(
      join(testDir, "wiki/concept/no-contribs.md"),
      `---\n${fm}\n---\n\nNothing yet.\n`,
    );
    ...
    while (Date.now() < deadline) {
      const wikis = await getWikis();
      if (wikis.find((w: any) => w.label === "No Contribution Wiki")) break;
      await new Promise((r) => setTimeout(r, 200));
    }

    const filtered = await fetch(
      `http://localhost:${API_PORT}/wikis?space_id=${spaceId}&has_contributions=true`,
    ).then((r) => r.json());

    expect(filtered.wikis.length).toBeGreaterThan(0);
    for (const w of filtered.wikis) {
      expect((w.properties.contributions as unknown[] | undefined)?.length ?? 0).toBeGreaterThan(0);
    }
    expect(
      filtered.wikis.find((w: any) => w.label === "No Contribution Wiki"),
    ).toBeUndefined();

    // Sanity: without the filter, the contribution-less wiki shows up.
    const all = await fetch(
      `http://localhost:${API_PORT}/wikis?space_id=${spaceId}`,
    ).then((r) => r.json());
    expect(
      all.wikis.find((w: any) => w.label === "No Contribution Wiki"),
    ).toBeTruthy();
  });

  it("include=counts returns the correct contributions_pending tally", async () => {
    const data = await fetch(
      `http://localhost:${API_PORT}/wikis?space_id=${spaceId}&include=counts`,
    ).then((r) => r.json());

    const race = data.wikis.find((w: any) => w.label === "Race Condition Test");
    expect(race).toBeTruthy();
    // The concurrency test added exactly 10 contributions to this wiki.
    expect(race.counts.contributions_pending).toBe(10);

    const noContribs = data.wikis.find((w: any) => w.label === "No Contribution Wiki");
    expect(noContribs).toBeTruthy();
    expect(noContribs.counts.contributions_pending).toBe(0);
  });
});
