...
The bundled writer template ships `cron: "*/15 * * * *"` (every 15 minutes). Operators override the cadence per role via `agents.yaml`; a role with no resolvable `cron:` simply doesn't auto-run.

Why no event queue? The cron model lets the role decide its own work from current state, which means new behaviors (reflector picking articles with open threads, bridger rotating across spaces) need only a different prompt + cron — no scheduler changes. Idempotency for cron runs is skipped at the runtime layer (no triggerPath = no key to dedupe against), so consecutive ticks aren't blocked by `agent_runs` short-circuiting.
...
- `agent_runs` — idempotency tracking for non-cron agent calls, keyed by `(role, idempotency_key)` with an `input_hash` so re-triggers on the same input skip cleanly. Cron-driven runs (no triggerPath) bypass this table — every tick is a fresh "look at state and decide."
...
- `embedding_queue` — per-entity work queue drained by the in-process embedding worker. Lease pattern (`started_at + 5min`) makes it crash-safe.

No actors, no auth, no versioning. Schema across `src/schema/001-foundation.sql` through `006-collapse-stubs.sql`. The pre-cron `agent_queue` table is gone from `001-foundation.sql` outright — fresh installs never see it; existing dev databases that still have the table can drop it manually or reset the database.
...
- `src/server/lib/embedding-queue.ts` — pure SQL helpers around `embedding_queue` (enqueue, claimNext, complete, fail, reclaimOrphans, queueStats, waitForDrain). Lease pattern + orphan reclaim mirror what the deleted `agent-queue.ts` used to do for events; the embedding queue stays because embeddings are still event-driven (each wiki sync enqueues one).
